...
ZunResult AnmManager::LoadTexture(i32 textureIdx, char *textureName, i32 textureFormat, ZunColor colorKey)
{
    ...
    u32 texW = BitCeil((u32)textureSurface->w);
    u32 texH = BitCeil((u32)textureSurface->h);

    this->textures[textureIdx].handle = this->currentTextureHandle;
    this->textures[textureIdx].textureData = rawTextureData;
    this->textures[textureIdx].width = texW;
    this->textures[textureIdx].height = texH;
    
    this->textures[textureIdx].surfW = textureSurface->w;
    this->textures[textureIdx].surfH = textureSurface->h;
...
cleanup:
    ...
    linearFree(dstFormatPixels);
}

//the previous implementation would generate a texture from the surface, draw and then discard it every time it was called
//this is very inefficient here because glTexImage2D uses vram/linearAlloc which is VERY expensive when called repeatedly
//so instead we just generate the texture when loading the surface (which acts as a cache) and then bind it here
